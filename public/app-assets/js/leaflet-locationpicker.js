(function (factory) {
    ...
    $.fn.leafletLocationPicker = function (opts, onChangeLocation) {
        ...
        var optsMap = {
            zoom: 13,
            center: L.latLng([5.562287, 95.334882]),
            zoomControl: false,
        ...
        function buildMap(self) {
            ...
            if (opts.mapContainer && $(opts.mapContainer))
                self.$map.appendTo(opts.mapContainer).addClass("map-select");
            ...
            if (typeof opts.layer === "string" && baseLayers[opts.layer])
                ...
                opts.layer instanceof L.TileLayer ||
            ...
            if (opts.alwaysOpen !== true) {
                ...
                xmap.onAdd = function (map) {
                    ...
                    var btn = L.DomUtil.create(
                        "a",
                        "leaflet-control " + opts.className + "-close"
                    );
                    btn.innerHTML = "&times;";
                    btn_holder.appendChild(btn);
                    L.DomEvent.on(btn, "click", L.DomEvent.stop, self).on(
                        btn,
                        "click",
                        self.closeMap,
                        self
    ...
    $.fn.leafletLoadLocation = function (opts, onChangeLocation) {
        var http = window.location.protocol;
